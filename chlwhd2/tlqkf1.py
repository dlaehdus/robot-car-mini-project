...
class KeyboardController:
    def __init__(self):
        ...
        self.STEERING_STEP = 30

        self.VELOCITY_MAX = 330
        self.VELOCITY_MIN = -330
        self.STEERING_MAX = 60
        self.STEERING_MIN = -60
        
        self.RETURN_VELOCITY = 5
        self.RETURN_STEERING = 15
...
class SteeringMotorController:
    def __init__(self, port_name='/dev/ttyACM0', baud_rate=1000000, steering_ids=(1, 2, 3, 4)):
        ...
        self.ROBOT_WIDTH = 385.5
        self.ROBOT_LENGTH = 470
    ...
    def update_steering(self, steering_angle):
        if steering_angle == 0:
            ...
            target_angles = [front_right, front_left, rear_right, rear_left]
    
        target_positions = [self.steering_to_position(a) for a in target_angles]
...
class InWheelMotorController:
    def __init__(self, port_list=("/dev/ttyACM1", "/dev/ttyACM2", "/dev/ttyACM3", "/dev/ttyACM4"),
                 velocity_ids=(0x01, 0x01, 0x01, 0x01)):
    ...
    def set_velocity_all(self, velocity):
        signs = [-1, -1, 1, 1]
        for ser, motor_id, sign in zip(self.serial_connections, self.velocity_ids, signs):
            target_velocity = sign * velocity
...
class MotorControlNode(Node):
    def __init__(self):
        ...
        self.steering_ctrl = SteeringMotorController(port_name='/dev/ttyACM0', steering_ids=(1,2,3,4))
    ...
    def control_loop(self):
        # 항상 전륜 구동
        self.iwc.set_velocity_all(self.kbc.velocity)
        if 'q' in self.kbc.keys_pressed:
            # q 유지 시 다이나믹셀 포지션 고정
            target=[1024,3072,3072,1024]
            for mid,pos in zip(self.sc.steering_ids,target):
                self.sc.packet.write4ByteTxRx(self.sc.port,mid,116,pos)
            status='q 모드 유지: 포지션 고정'
        else:
            angles=self.sc.update_steering(self.kbc.steering)
            status=(f"속도: {self.kbc.velocity} | FR:{angles[0]:.1f}°, FL:{angles[1]:.1f}°, "
